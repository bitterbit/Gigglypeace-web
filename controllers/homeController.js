...
// create the controller and inject Angular's $scope
	...
	// create the controller and inject Angular's $scope
	angular.module('jigglyApp').controller('homeController', function($scope,$http) {
	 // create a message to display in our view
	
	 $scope.discussions = [
	{title :'woho', description : 'I like Jigglipeace',	publisher: 'Eitan Bolbol Gadol',
	 			origin_lang:'he' ,number_of_views:50,
			image_link:'https://encrypted-tbn2.gstatic.com/images?q=tbn:ANd9GcQ5zV9xuZjieKMGmEHa1t7rzWjhlx2penwXNA5Ka5GcvLCzznzI4A',
			sideA :'side a',sideB:'side b',sideACount:50,sideBCount:40,neutralCount:30},
	 
	 ];
	 $http.get('/api/discussions?from=' +0 + '&to=' + 1).success(function (data, status, headers, config) {
        console.log(data);
